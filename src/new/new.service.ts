...
private async findTeacherByIdOrThrowExp ( teacher_Id : number) {
  ...
  async create(createNewDto: CreateNewDto , file : Express.Multer.File) {
    ...
    let mGroups:Group[] = [];
// finding the groups 
   for (let x = 0 ; x<fakeGroups.length; x ++) { 
       let group = await this.findGroupByIdOrThrowExp(fakeGroups[x]);
       mGroups.push(group)
     }
// groups are founded
    ...
    let mNew = await this.newsRepository.create({message : createNewDto.message});
     ...
     let fileName = 'spec'+ uuidv4() + fileExtinction ;
  ...
  async findAllNewsOf_a_Group ( group_id : number ) {
    ...
    try {
      ...
      query(
        `SELECT id AS newsId ,group_Id , message , file , teacher_Id , created_at , approved_date
        FROM groupsViewNews INNER JOIN news ON groupsViewNews.new_Id = news.id
        WHERE approved AND groupsViewNews.group_Id = ${group_id};`
        )
  ...
  public async news_To_Approve ( ) { 
     try {
       let newsToApprove = await this.newsRepository.query(
         ` SELECT news.id as new_Id , message , file , teacher_Id , news.created_at FROM 
